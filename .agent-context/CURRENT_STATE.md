...
`npm test` now includes `tests/responsive_site_check.py`, which audits every exported public `index.html` for viewport metadata, media-query/fluid sizing CSS, responsive navigation chrome, profile/article layout markers, and homepage-specific mobile overflow guard markers for the hero/search/status/CTA area. After responsive changes, also smoke-check all exported public routes from `out/`, not only a few representative pages; for visual mobile issues, capture or request an actual narrow-viewport screenshot because structural markers alone can miss clipping.
...
- Data-driven robot/company profile renderers now use an imported-frontend-style shell (`hd-profile`, `hero-inner`, quick facts, sticky/mobile TOC, source-backed sections) instead of the earlier generic React profile layout, so enriched profiles visually belong with the approved frontend.
...
- Global CSS now includes responsive breakpoints for desktop, tablet, and mobile layouts across navigation, cards, articles, generated profiles, facts bars, sidebars, sources, and footers.

The seed robot/company data still needs deeper source verification and richer per-entity fields before full launch. Tesla Optimus/Tesla, Figure 01/Figure AI, Figure 02/Figure AI, Unitree G1/Unitree Robotics, Unitree H1/Unitree Robotics, Digit/Agility Robotics, Apollo/Apptronik, 1X EVE/1X Technologies, 1X NEO/1X Technologies, Sanctuary AI Phoenix/Sanctuary AI, Boston Dynamics Electric Atlas/Boston Dynamics, Fourier GR-1/Fourier Intelligence, UBTECH Walker S/UBTECH Robotics, EngineAI PM01/EngineAI, and PAL Robotics TALOS/PAL Robotics now have enriched/source-backed passes with sources, timeline entries, cautious status/availability wording, and visible program notes. TALOS is tracked as a commercially offered research humanoid platform with quote-based availability and public PAL/IEEE specs; EVE is tracked as a wheeled enterprise humanoid predecessor/pilot-deployment lineage with reported public specs; NEO remains the current home-humanoid pre-order/early-access direction. Figure 01 is tracked as a discontinued/superseded predecessor with reported public specs; Figure 02 and Figure AI still preserve the imported HTML visual pages, with appended source-backed status panels to keep the gold-standard visual treatment while resolving current-status drift.
